...
export default function FriendExpensesScreen() {
    ...
    try {
      ...
      // e 'friend_id' é o amigo da página atual.
        ...
        console.log(`[SupabaseFetch] Resposta Supabase (despesas) - Status: ${status}, Erro:`, supabaseError);
    ...
    const monthSections = Object.keys(groupedExpenses).sort((a, b) => {
        ...
        const dateA = new Date(parseInt(yearA, 10), new Date(Date.parse(monthA + " 1, 2012")).getMonth());
        const dateB = new Date(parseInt(yearB, 10), new Date(Date.parse(monthB + " 1, 2012")).getMonth());
        return dateB.getTime() - dateA.getTime();
    }); // Ordenar por data mais recente

    // Ecrã de Loading inicial
    if (auth.isLoading) {
        return <View style={[styles.loadingContainer, { paddingTop: insets.top }]}><ActivityIndicator size="large" color="#007AFF" /><Text>A verificar autenticação...</Text></View>;
    }
    if (!auth.user) {
        return <View style={[styles.loadingContainer, { paddingTop: insets.top }]}><Text style={styles.errorText}>Precisa de estar logado.</Text></View>;
    }
    if (loading && expenses.length === 0) {
        return <View style={[styles.loadingContainer, { paddingTop: insets.top }]}><ActivityIndicator size="large" color="#007AFF" /><Text>A carregar despesas com {friendName}...</Text></View>;
    }
    ...
    <View style={[styles.outerContainer, { paddingTop: insets.top }]}>
        ...
        <View style={styles.customHeader}>
            ...
            <Text style={[styles.headerBalanceSummary, balanceSummaryColor]}>{balanceSummaryText}</Text>
            </View>
            {/* </ImageBackground> */}
        ...
        >
            ...
            <View key={monthYear} style={styles.monthSection}>
                <Text style={styles.monthYearText}>{monthYear.charAt(0).toUpperCase() + monthYear.slice(1)}</Text>
                {groupedExpenses[monthYear].map((expense) => {
                    const expenseDate = new Date(expense.date);
                    const day = expenseDate.getDate();
                    const monthAbbrev = expenseDate.toLocaleString('pt-PT', { month: 'short' }).replace('.','');
                    ...
                    return (
                        ...
                        <View style={styles.expenseIconContainer}>
                            <Ionicons name={expense.category_icon || "receipt-outline"} size={24} color="#4F4F4F" />
                        </View>
                        <View style={styles.expenseDetails}>
                            <Text style={styles.expenseDescription} numberOfLines={1} ellipsizeMode="tail">{expense.description}</Text>
                            <Text style={styles.paidByText}>
                                {expense.paid_by_user ? `Pagou ${expense.total_amount.toFixed(2)} €` : `${friendFirstName} pagou ${expense.total_amount.toFixed(2)} €`}
                            </Text>
                        </View>
                        <View style={styles.expenseShareContainer}>
                            <Text style={[styles.expenseShareAmount, shareColor]}>{userShareAbs} €</Text>
                            <Text style={[styles.expenseShareLabel, shareColor]}>{shareText}</Text>
                        </View>
                        </View>
                    );
                })}
...
const styles = StyleSheet.create({
    ...
    positiveAmountColor: { // Emprestou (amigo deve)
        color: '#27ae60',
    },
    negativeAmountColor: { // Emprestaram-lhe (você deve)
        color: '#e74c3c',
    },
    noExpensesText: {
        textAlign: 'center',
